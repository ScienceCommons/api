...
describe ReplicationsController, :type => :controller do
  ...
  describe('#index') do
    it("returns a list of all replications for a study") do
      ...
      results.should include(JSON.parse(replication_1.as_json(replications: true, authors: true, year: true, comments: true).to_json))
      results.should include(JSON.parse(replication_2.as_json(replications: true, authors: true, year: true, comments: true).to_json))
    end
  ...
  describe('#show') do
    it("should return a single replication") do
      ...
      response.status.should == 200
      JSON.parse(response.body).should == JSON.parse(replication_1.as_json(replications: true, authors: true, year: true, comments: true).to_json)
    end
